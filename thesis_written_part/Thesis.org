...
*** Bayesian Parameter Learning - An Exponential Family Generalization
    ...
    general exponential family distributions. We will see next that
    when choosing conjugate priors we will get as posteriors well
    known distributions such that we will get the analytical solution
    for the maximum of such distribution from standard statistical
    theory available in any basics statistical book.

    Given such theory we will allow the general usage of exponential
    families with conjugate priors such that a vast multitude of
    modeling possibilities will be given to the end user. 
