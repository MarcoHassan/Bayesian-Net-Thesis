...
\label{sec:orged9330e}

\begin{itemize}
\item say that the major issues are in the computational aspects which
were just marginally touched in this manuscript

\item say that you could improve the computational issues also by
propagating with approximate inference. (actually you have to
think if it satisfies the EM-conditions properly).
\end{itemize}

\subsection{Further Work}
\label{sec:org197b20c}

\begin{itemize}
\item you can try to implement the numerical solution proposed in
chapter 6 and explore its time efficiency.

\item use further approximate learning algorithms.
\end{itemize}


\subsection{{\bfseries\sffamily TODO} Say that you can also work with approximate inference Methods. Does not change the story.}
\label{sec:org6e5014f}

\newpage

\section{Conclusion}
\label{conclusion}
\subsection{{\bfseries\sffamily TODO} Focus on the Point the point that no matter}
\label{sec:org4b1762b}
